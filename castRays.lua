...
local function castRays(
	...
	for _, direction in directions do
		...
		if raycastResult then
			...
			rayResult.instance = raycastResult.Instance.Parent

			local humanoid = raycastResult.Instance.Parent:FindFirstChildOfClass("Humanoid")
			local humanoid2 = raycastResult.Instance.Parent.Parent:FindFirstChildOfClass("Humanoid")
			if humanoid and canPlayerDamageHumanoid(player, humanoid) then
				rayResult.taggedHumanoid = humanoid
				rayResult.taggedCharacter = humanoid:FindFirstAncestorOfClass("Model")
			end
			if humanoid2 and canPlayerDamageHumanoid(player, humanoid2) then
				rayResult.taggedHumanoid = humanoid2
				rayResult.taggedCharacter = humanoid2:FindFirstAncestorOfClass("Model")
			end
		end
